...
import (
	...
	"grapefruit/internal/adapter/mongo/builder"
	"grapefruit/internal/adapter/mongo/field"
)
...
func NewCollection(db *mongo.Database, name string, opts ...*options.CollectionOptions) *Collection {
	...
	return &Collection{Collection: coll, database: db}
}

func CollectionByName(db *mongo.Database, name string, opts ...*options.CollectionOptions) *Collection {
